	...
	\subsection{Optimisation problem}
		...
		\begin{frame}
			...
			\begin{itemize}
				...
				\item<1-> When $g(\varrho)$ is defined as \emph{\color{blue}infimum over the convex roof}, the $2^{\rm nd}$ optimisation simplified to pure states only,
					\begin{block}
						{}
						{\small
						\vspace{8px}
						\[
						g(\varrho)\mathcal{B}(w) = \sup_{r} \big( r w - \sup_{\ket{\psi}} [ r\expect{W} - g(\ket{\psi}) ] \big).
						\]}
					\end{block}
			\end{itemize}
...
\section{Case study}
		...
		\begin{frame}
			\begin{itemize}
				\item<1-> We apply our method to projectors with great success.
				\vspace{10px}
				...
				\item<3-> Though, in this presentation we will focus on collective $J_{\mathbf n}$ operators.
			\end{itemize}
