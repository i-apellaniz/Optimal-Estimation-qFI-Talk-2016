...
\section{Introduction and Motivation}
	...
	\begin{frame}
		\begin{enumerate}
			...
			\only<4->{
			\item	The archetypical criteria that demonstrates \emph{\color{blue}metrologicaly useful entanglement} on the state.
				\only<4>{
...
\section[QFI based on expectation values]{QFI based on expectation values: Are they optimal?}
	...
	\subsection{Optimisation problem}
		...
		\begin{frame}
			\frametitle{Optimisation: Legendre Transform}
			\begin{itemize}
				\item<1-> For a convex function of the state, we construct a \emph{\color{blue}thight lower bound} as follows,
					{\small \[
					g(\varrho) \geq
					\mathcal{B} \big( \{w_k := \expect{W_k}\} \big) = \sup_{\{r_k\}} \big( r \cdot w - \sup_{\varrho} [ r\cdot\expect{W}_{\varrho} - g(\varrho) ] \big).
					\]}

					\only<1>{\hfill
					\begin{minipage}{250px}
						\begin{flushright}\color{gray}
							The scalar product reads as follows $r \cdot w \equiv \sum_k r_k w_k$ and $r\cdot\expect{W}_{\varrho} \equiv \sum_k r_k \expect{W_k}_{\varrho}$.
						\end{flushright}
					\end{minipage}
					\vspace{-22px}
					}
				\item<2-> When $g(\varrho)$ is defined as \emph{\color{blue}infimum over the convex roof}, the $2^{\rm nd}$ optimisation simplified to pure states only,
					\begin{block}
						{}
						{\small
						\vspace{8px}
			...
			\onslide<2->{\citate{O. G\"uhne, M. Reimpell \& R.F. Werner, PRL \textbf{98}, 110502 (2007)}
		...
		\begin{frame}
			\begin{block}
				...
				The \emph{\color{blue} simplicity} of qFI for pure states allows to rewrite the problem:
				{\small
				\[
				\mathcal{F}(\{w_k\}) = \sup_{\{r_k\}} \big( r\cdot w - {\color{blue}\sup_{\mu} [ \lambda_{\max} ( r\cdot W - 4(J_z-{\color{blue}\mu})^2 ) ]} \big).
				\]}
			\end{block}
			\begin{itemize}
				\item Therefore, we have parametrised the optimisation, which leads to a \emph{\color{blue}more efficient finding} of the solution.
...
\section{Case study}
		...
		\begin{frame}
			\begin{itemize}
				...
				\item<1-> Though, we apply our method to projectors with great success, we will focus on collective $J_{\mathbf n}$ operators.
				\vspace{10px}
				...
				\item<2-> One of the cases using projector operators, {\it i.e.}, using the \emph{fidelity}, leads to \emph{\color{blue}analytic soulution}!
			\end{itemize}
